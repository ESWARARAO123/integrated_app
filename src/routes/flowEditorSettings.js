...
// Apply authentication middleware to all flow editor settings routes
router.use(authenticateToken);

/**
 * Get user's flow editor settings
 * GET /api/flow-editor-settings
 */
  ...
  try {
    ...
    if (result.rows.length === 0) {
      ...
      const defaultSettings = {
        ...
        central_scripts_directory: null,
        mcp_server_id: null,
        mcp_server_url: null,
...
router.post('/', async (req, res) => {
  try {
    ...
    if (existingSettings.rows.length > 0) {
      ...
      // Insert new settings - will continue in next part due to line limit
      result = await pool.query(`
    ...
    res.json({
      message: 'Flow editor settings saved successfully',
 ...
 * DELETE /api/flow-editor-settings
 */
